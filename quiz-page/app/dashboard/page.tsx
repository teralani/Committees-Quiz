...
export default async function DashboardPage() {
  ...
  return (
    <div className={`${montserrat.variable} font-sans min-h-screen bg-slate-50`}>
      <nav className={`h-20 md:h-16 flex justify-center items-center w-full bg-${CONFERENCES.indexOf(conference) != -1? conference : "kingmun"}-primary text-white`}>
        <div id="LOGO" className="hidden md:block mr-4" />
        <h1 className="text-2xl font-bold">Committee Quiz Editor</h1>
        <form action={logout}>
          <button
            className="cursor-pointer px-4 py-2 absolute right-8 top-3 bg-rose-500 text-white rounded"
          >
            Log out
          </button>
